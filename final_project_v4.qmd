...
max_row <- which.max(movie_data$budget_2013)
# Drop that row
movie_data <- movie_data[-max_row, ]
nrow(movie_data)
```
...
ggplot(movie_data, aes(x = budget_2013_millions, y = predicted_prob, color = decade_code)) +
  ...
  geom_smooth(method = "loess", se = TRUE, color = "black") +  # Trend line
...
Interpretation: The plot shows that the 1980s ('80-'89) display a different trend compared to other decades in the relationship between budget and the predicted probability of passing the Bechdel Test. The following is how we dig into the different trend:
