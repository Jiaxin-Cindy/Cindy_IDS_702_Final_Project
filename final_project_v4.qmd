...
```{r, message=FALSE, results='hide'}
ResearchQ2Model <- lm(intgross_2013 ~ binary * genre + budget_2013 + year, data = movie_data)

summary(ResearchQ2Model)

plot(ResearchQ2Model)
```

```{r}
#Conduct Log Transformation

movie_data$log_budget <- log(movie_data$budget_2013)
movie_data$log_intgross <- log(movie_data$intgross_2013)
log_lm_model <- lm(log_intgross ~ binary * genre + log_budget + year, data = movie_data)
