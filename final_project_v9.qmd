...
Media equity and cultural narratives are significantly impacted by gender representation in films. This study investigates the relationship between movie budgets and their likelihood of passing the Bechdel Test, as well as differences in worldwide box office revenue between movies that pass or fail the test. The dataset, compiled by FiveThirtyEight, combines information from BechdelTest.com and The-Numbers.com, with movie genres supplemented using IMDb. This study investigates the relationship between movie budgets and their likelihood of passing the Bechdel Test, as well as the impact of passing the test on international box office revenue. Logistic regression revealed a negative relationship between movie budgets and the probability of passing the Bechdel Test, with higher-budget films less likely to prioritize gender-inclusive narratives. Temporal trends show increased inclusivity over recent decades, with significant decade and genre effects observed. Linear regression indicated that production budgets and genres are the strongest predictors of international gross revenue, while passing the Bechdel Test did not have a statistically significant effect on financial outcomes. These findings highlight the nuanced role of budget, genre, and decade in shaping gender representation and revenue trends in cinema.
...
Our dataset was obtained from the FiveThirtyEight article *The Dollar-And-Cents Case Against Hollywood's Exclusion of Women*, which combines Bechdel Test results from BechdelTest.com and financial metrics from TheNumbers.com. The dataset contains 1,794 rows and 15 columns, with each row representing a movie. It includes whether the movie passed the Bechdel Test, its budget, and its domestic and worldwide revenues, reported in both release year dollars and inflation-adjusted 2013 dollars.

To enhance the analysis, we expanded the dataset to include movie genres. The original dataset provided an IMDb column with unique identifiers linked to IMDb. Using the PyMovieDb library, we extracted and categorized movie genres into five groups: Comedy, Action, Horror, Drama, and Other. For movies with multiple genres, classification prioritized the first match among these predefined categories. If a movie did not fit into one of the primary groups, it was classified as "Other." We initially included Romance and Family genres as well, but due to low n counts, these movies were combined into the Other category.

During data processing, we chose to retain only the inflation-adjusted 2013 USD columns for budget, domestic revenue, and worldwide revenue to ensure consistent financial comparisons. Initial exploration revealed missing values in several key columns. Specifically, the domestic gross revenue and international gross revenue columns had missing values for 18 and 11 rows, respectively, with some overlap. The cause of these missing values was unclear and all of the movies in our dataset were American movies. Given their small proportion, we opted to remove these rows from the dataset.
...
**Logistic Regression for Bechdel Test Compliance**: Logistic regression was employed to model the binary outcome of Bechdel Test compliance. Predictors included the movie’s production budget (2013 USD), the decade of release, allowing exploration of how the relationship between budget and Bechdel Test compliance evolved over time, as well as genre and worldwide revenue. Predictors were determined based on AIC and VIF evaluation. Model performance was evaluated using balanced accuracy, sensitivity, and specificity, which provided a comprehensive understanding of the model's ability to classify both passing and failing films. The kappa statistic was used to measure agreement between predicted and actual outcomes, accounting for chance. Predicted probabilities from the logistic regression model offered insights into the effects of budget, decade, worldwide revenue and genre on the likelihood of passing the Bechdel Test.

**Linear Regression for International Box Office Revenue:** To address the second research question, a linear regression model was developed to explore the factors influencing a movie’s worldwide box office revenue. Key predictors included compliance with the Bechdel Test (binary variable), production budget, genre, and decade of release. The log transformation of budget and worldwide revenue was applied to normalize their distributions and mitigate the effect of extreme values, ensuring better model performance. The model was tested both with and without an interaction term between the binary Bechdel Test compliance variable and the movie's genre. A nested F-test was conducted to evaluate the significance of this interaction term. Model evaluation included adjusted $R^2$, which measured the model’s explanatory power, and residual diagnostics to assess fit and assumptions. Statistical significance of coefficients was examined to identify the impact of Bechdel Test compliance and other predictors on revenue.
...
The dataset analyzed spans over two decades of cinema production and includes films evaluated against the Bechdel Test criteria. In total, 1,776 films were included in the analysis after initial preprocessing, representing a wide array of genres, production budgets, and revenue outcomes. Of these, 44.7% passed the Bechdel Test, indicating that they featured meaningful interactions between female characters, while the remaining 55.3% failed.
...
```{r, message=FALSE, fig.show='hide',results='hide'}
# testing a couple of different combinations of models here to ensure the variables we selected is the best combo, based on AIC
# we are determining what variables we may want to include in the model in addition to those we are interested in in the research question

mod_test_1 <- glm(binary ~ budget_2013 + decade_code + genre + domgross_2013 + intgross_2013, data = movie_data, family = "binomial") 

mod_test_2 <- glm(binary ~ budget_2013 + decade_code + domgross_2013 + intgross_2013, data = movie_data, family = "binomial")

mod_test_3 <- glm(binary ~ budget_2013 + decade_code + genre + intgross_2013, data = movie_data, family = "binomial")

mod_test_4 <- glm(binary ~ budget_2013 + decade_code + genre + domgross_2013, data = movie_data, family = "binomial")

mod_test_5 <- glm(binary ~ budget_2013 + decade_code, data = movie_data, family = "binomial")

mod_test_6 <- glm(binary ~ budget_2013 + decade_code+ genre, data = movie_data, family = "binomial")

mod_test_7 <- glm(binary ~ budget_2013 + decade_code + domgross_2013, data = movie_data, family = "binomial")

mod_test_8 <- glm(binary ~ budget_2013 + decade_code + intgross_2013, data = movie_data, family = "binomial")
...
```{r,message=FALSE, fig.show='hide',results='hide'}
# we want to ensure that international revenue is inclusive of domestic revenue
# Filter rows where domestic revenue is greater than international revenue
greater_domestic_revenue <- movie_data[movie_data$domgross_2013 > movie_data$intgross_2013, ]

# View the rows where the condition is true
print(greater_domestic_revenue)

# If you just want to count how many rows meet the condition:
count <- sum(movie_data$domgross_2013 > movie_data$intgross_2013)
cat("Number of cases where domestic revenue is greater than international revenue:", count, "\n")

head(movie_data)

# through researching a couple of sample movies it is clear that international revenue includes domestic
```

```{r,message=FALSE, fig.show='hide',results='hide'}
# Construct the logistic regression model
glm_model <- glm(binary ~ budget_2013 + decade_code + genre + intgross_2013, data = movie_data, family = "binomial")
...
Figure 1 below illustrates the predicted probability of a film passing the Bechdel Test as a function of its production budget (adjusted to 2013 USD), stratified by decade. The black trendline indicates an overall negative relationship between budget and the likelihood of passing the Bechdel Test, with probabilities decreasing as budgets increase up to approximately \$300 million.
...
ggplot(average_revenue, aes(x = genre, y = mean_revenue, fill = binary)) +
  ...
  labs(
    ...
    x = "Genre",
    y = "Avg. WorldWide Gross Revenue (Millions 2013 USD)",
...
```{r, message=FALSE, results='hide'}
mod_full <- lm_mod_decade_code
mod_reduced <- lm(intgross_2013 ~ binary + genre + budget_2013 + decade_code, data = movie_data)
anova(mod_reduced, mod_full, test="F")

```

Now that it was determined an interaction term would not be included, we moved forward with the variables of Bechdel test outcome, genre, budget and decade code in our model. This linear model was created, but creation of diagnostic plots revealed a fanning pattern in the residuals vs. fitted plots as well as significant deviation from the theoretical quantile line in the QQ-plot, indicating the presence of heteroscedasticity and non-normal distribution of the residuals respectively. In order to combat this, we performed a logarithmic transformation on the numerical variables in our model, worldwide revenue and budget. This transformation dramatically improved the previous issues seen in the diagnostic plots. As seen in figure 3 below the Residuals vs. Fitted plot reveals a reasonable scatter around zero, though slight heteroscedasticity at higher fitted values indicates possible variability in error variance. The Q-Q plot demonstrates approximate normality of residuals, with minor deviations in the tails suggesting some outliers or non-normal behavior. These diagnostics indicate the model is well-specified, though further refinement may improve robustness.
...
plot(lm_mod_decade_code)
```
**Figure 3:  Regression model diagnostics plot**
...
Our final linear model yielded the following results, shown in table 4  below
...
conf_intervals <- confint(log_lm_model, level = 0.95)  # Default is 95% confidence level
...
The linear regression model reveals that the natural log of a movie’s budget is the strongest predictor of international gross revenue, with a highly significant positive association (estimate = 0.895, p \< 0.001). Among genres, horror movies significantly outperform the reference genre (estimate = 0.672, p \< 0.001), while dramas show a modest but significant negative impact (estimate = -0.186, p = 0.026). Passing the Bechdel Test does not significantly influence revenue (estimate = -0.056, p = 0.368), suggesting no clear relationship between gender representation and financial success in this model. Decade effects indicate a steady decline in international gross revenue over time, with the 2000s (estimate = -1.318, p \< 0.001) and 2010s (estimate = -1.234, p \< 0.001) showing the largest negative associations. These findings highlight budget, genre, and decade as key factors in predicting international gross revenue.
...
# **Figure 6:** Histogram of Worldwide Gross Revenue (2013 dollars, in millions)

# Convert worldwide gross revenue to millions
movie_data <- movie_data %>%
  mutate(intgross_2013_millions = intgross_2013 / 1e6)

# Histogram for worldwide gross revenue in millions
ggplot(movie_data, aes(x = intgross_2013_millions)) +
  geom_histogram(fill = "steelblue", bins = 30) +
  labs(
    x = "Worldwide Gross Revenue (2013 dollars, in millions)",
    y = "Frequency"
  ) +
  ggtitle("") +
  theme_minimal()
...
ggplot(average_revenue, aes(x = binary , y = mean_revenue, fill = genre)) +
  ...
  labs(
    ...
    y = "Average Worldwide Gross Revenue (Millions 2013 USD)",
    fill = "Genre"
  ) +
  theme_minimal() +
  theme(
...
ggplot(movie_data, aes(x = genre , y = log_intgross_2013, fill = genre)) +
  ...
  scale_y_continuous(labels = scales::dollar_format(suffix = "M")) +
  labs(
    title = "Worldwide Gross Revenue by Bechdel Test Result and Genre",
    x = "Bechdel Test Result",
    y = "Worldwide Gross Revenue (2013 dollars, in millions)",
    fill = "Bechdel Test Result"
  ) +
  theme_minimal() +
  theme(
...
This study provides insights into the likelihood of passing the Bechdel Test and its relationship with movie budgets, as well as differences in worldwide box office revenue between movies that pass or fail the test. Using the FiveThirtyEight dataset and supplemental IMDb movie details, key insights were revealed, highlighting the connection between gender representation and financial investments in the film industry. The analysis shows that passing the Bechdel Test is influenced by factors such as the movie’s budget, production decade, and its classification as an Action or Horror genre. In terms of worldwide revenue, a positive relationship was observed with production decades and the prominence of Horror and Drama genres. However, the research also revealed that the results of the Bechdel Test are not significantly associated with worldwide revenue, indicating not significant relationship between gender representation and financial success.
