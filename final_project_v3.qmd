...
significant_results <- data.frame(
  ...
  Estimate = round(lm_summary$coefficients[, "Estimate"], 4),
  Std_Error = round(lm_summary$coefficients[, "Std. Error"], 4),
  t_value = round(lm_summary$coefficients[, "t value"], 4),
  p_value = lm_summary$coefficients[, "Pr(>|t|)"]
)

# Filter only statistically significant variables (p-value < 0.05)
significant_results <- significant_results[significant_results$p_value < 0.05, ]

# Replace interaction term format and improve readability
significant_results$Variable <- gsub(":", " * ", significant_results$Variable) # Replace colons with asterisks for interaction terms
significant_results$Variable <- gsub("_", " ", significant_results$Variable)  # Replace underscores with spaces
significant_results$Variable <- gsub("`", "", significant_results$Variable)  # Remove backticks

# Use knitr::kable to format the table in LaTeX
knitr::kable(
  significant_results,
  format = "latex",
  caption = "Linear Regression Model Summary: Significant Coefficients",
  align = c("l", "c", "c", "c", "c"), # Left-align the Variable column, center others
  booktabs = TRUE,
  linesep = "",
  row.names = FALSE # Suppress row names
