...
Across most decades, there is a negative relationship between movie budget and the predicted probability of passing the Bechdel Test, indicating that higher-budget films are generally less likely to pass. The results of the logistic regression model are shown in tables 2 and 3 below:
...
In order to select which variables to include in this model in addition to the variables of interest from our research question, the AIC value of models with varied combinations of variables were calculated. The model with the lowest AIC value was that which included genre, domestic revenue and international revenue. AIC punishes the inclusion of additional predictors, so we felt this was an accurate way for us to select the best model. However, examining VIF values revealed high multicollinearity between domestic and international revenue, which we concluded was due to international revenue including domestic; therefore we included only international revenue in the final model.

The logistic regression analysis reveals key factors influencing the likelihood of a film passing the Bechdel Test. Budget is a significant predictor, with higher budgets decreasing the odds of passing the test (p \< 0.001), suggesting that more expensive films may prioritize traditional narratives less aligned with diversity benchmarks. Decade effects are notable, with films from the 1990s, 2000s, and 2010s significantly more likely to pass the Bechdel Test compared to the baseline (likely pre-1980s), highlighting a positive trend in gender representation over time. Genre plays a critical role, as action films are significantly less likely to pass (p \< 0.001), whereas horror films are more likely to do so (p = 0.002). Drama and "Other" genres show no significant difference from the baseline genre. Interestingly, domestic revenue negatively correlates with passing the test (p = 0.027), while international revenue positively correlates (p = 0.022), indicating that films with inclusive narratives may perform better in global markets. These findings emphasize the influence of budget, genre, and temporal trends on gender representation in cinema, with international audiences appearing to reward inclusivity.
...
**Figure 3: Linear Regression Model Diagnostic Plots**
